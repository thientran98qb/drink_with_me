    ...
    return (
        <div className="bg-card rounded-3xl shadow-lg border border-border overflow-hidden">
            ...
            <div className="grid grid-cols-7 border-b border-border bg-muted/30">
                {weekDays.map((day) => (
                    <div key={day} className="py-4 text-center text-sm font-bold text-muted-foreground uppercase tracking-widest">
                        {day}
                    </div>
                ))}
            </div>
...
function DayCell({ date, currentMonth, onClick }: { date: Date, currentMonth: Date, onClick: () => void }) {
    ...
    return (
        <div
            className={cn(
                "min-h-[140px] p-2 sm:p-3 flex flex-col transition-colors relative",
    ...
    if (status.type === 'wife') {
        return (
            <>
                ...
                <div className="flex-1 flex flex-col gap-1.5 relative z-10 mt-2">
                    <div className="px-2 py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30">
                        <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400">
                            <status.icon className="h-4 w-4" />
                            <span className="text-[10px] sm:text-xs font-bold leading-tight">{status.label}</span>
                        </div>
                    </div>
                </div>
            </>
        ...
        <div className="flex-1 flex flex-col gap-1.5">
            {status.type === 'free' || status.type === 'maybe' ? (
                <button
                    onClick={onClick}
                    className={cn(
                        "w-full text-left px-2 py-2 rounded-xl border hover:scale-[1.02] active:scale-95 transition-all cursor-pointer",
                        status.type === 'free'
                ...
                >
                    <div className={cn("flex items-center gap-1.5 mb-1",
                        ...
                        <status.icon className={cn("h-4 w-4", status.type === 'free' && "animate-pulse")} />
                        <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wide">
                            {status.type === 'free' ? 'Rảnh' : 'Hên xui'}
                        </span>
                    </div>
                    <p className="text-xs sm:text-sm font-bold leading-tight text-foreground">{status.label}</p>
                </button>
            ) : (
                <div className={cn(
                    "px-2 py-2 rounded-xl border opacity-80",
                    ...
                    <p className={cn("text-xs sm:text-sm font-bold leading-tight text-foreground", status.type === 'busy' && "line-through")}>
                        {status.label}
                    </p>
                </div>
            )}
